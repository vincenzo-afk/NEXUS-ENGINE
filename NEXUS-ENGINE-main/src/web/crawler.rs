...
pub struct CrawlReport {
    ...
    /// Number of pages skipped because `classify::safety` flagged them
    /// (explicit/phishing/malicious/scam content, or an explicit
    /// blocklist match) before they were ever indexed.
    pub pages_skipped_unsafe: usize,
...
pub struct WebCrawler {
    ...
    policy_classifier: crate::classify::safety::PolicyClassifier,
}
...
impl WebCrawler {
    ...
    pub fn with_tor(config: WebCrawlConfig, tor_config: Option<TorConfig>) -> Result<Self> {
        ...
        Ok(WebCrawler {
            ...
            tor_config,
            policy_classifier: crate::classify::safety::PolicyClassifier::new(),
        })
    }
    ...
    pub fn crawl(
        ...
        while report.pages_fetched < self.config.max_pages {
            ...
            *pages_per_domain.entry(entry.domain.clone()).or_insert(0) += 1;
            ...
            // Populated by the classify pass in the `Html` arm below; other
            // formats have no HTML to run the heuristics on, so they stay
            // at the neutral "not spam, not flagged" default rather than
            // being penalized for a check that was never actually run.
            let mut page_spam_score: f32 = 0.0;
            let mut page_policy_flag: Option<crate::webdoc::PolicyFlagRecord> = None;
            ...
            let (indexable_text, discovered_links, title, meta_description, lang, author, feed_urls) =
                match format {
                    DocumentFormat::Html => {
                        ...
                        let spam_verdict =
                            crate::classify::spam::SpamClassifier::classify(&extracted, &entry.domain);
                        let policy_verdict =
                            self.policy_classifier.classify(&entry.url, &extracted);
                        page_spam_score = spam_verdict.score;
                        page_policy_flag = policy_verdict.flags.iter().max_by(|a, b| {
                            a.confidence
                                .partial_cmp(&b.confidence)
                                .unwrap_or(std::cmp::Ordering::Equal)
                        }).map(|f| crate::webdoc::PolicyFlagRecord {
                            category: f.category.label().to_string(),
                            confidence: f.confidence,
                        });
                        if policy_verdict.should_filter() {
                            debug!(
                                "{}: filtered by safety classifier ({} flag(s), blocklisted={})",
                                entry.url,
                                policy_verdict.flags.len(),
                                policy_verdict.blocklisted
                            );
                            report.pages_skipped_unsafe += 1;
                            continue;
                        }
            ...
            let metadata = DocumentMetadata {
                ...
                modified_unix: now,
                token_count: 0,
                // Crawled web content is public by construction — the
                // crawler already fetched it over plain HTTP(S)/Tor with
                // no credentials, so there's no "owner" for it to be
                // private to.
            ...
            let web_meta = WebPageMeta {
                ...
                incoming: Vec::new(),
                pagerank: 0.0,
                spam_score: page_spam_score,
                policy_flag: page_policy_flag.clone(),
            };
