...
/// Flags a claim whose negation status (contains a negation word or not)
/// disagrees with the cited span's, *when* the two otherwise share
/// substantial vocabulary (high overlap but opposite polarity is exactly
/// the "the source said the opposite" failure mode this exists to catch;
/// low overlap is already caught by the overlap threshold on its own).
fn negation_conflict(
    ...
    let claim_negated = NEGATION_WORDS.iter().any(|w| claim_lower.contains(w));
    let span_negated = NEGATION_WORDS.iter().any(|w| span_lower.contains(w));
